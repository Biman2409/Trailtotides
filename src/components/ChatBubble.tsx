  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
        content:
          "Hey! I'm TRAIL TO TIDES — tell me what kind of adventure you're after and I'll point you towards it.",
    },
  ]);
    ...
    <>
      ...
      {open && (
        ...
        >
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto p-6 space-y-6 max-h-[450px] custom-scrollbar">
            {messages.map((msg, i) => (
              <div
                key={i}
              ...
              >
                <div className="max-w-[90%] space-y-3">
                  {msg.content && (
                    <div
                      className={`px-5 py-3.5 rounded-2xl text-[14px] leading-relaxed shadow-lg ${
                        msg.role === "user"
                          ? "text-white rounded-tr-none font-bold"
                          : "bg-white/[0.04] border border-white/[0.08] text-white/85 rounded-tl-none font-light"
                      }`}
                      ...
                      {msg.cards.map((card, ci) => {
                        ...
                        return (
                          <Link
                            ...
                            className="flex items-stretch bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.06] hover:border-[#ff5100]/40 rounded-2xl overflow-hidden transition-all duration-500 group shadow-lg"
                          >
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img
                              src={card.heroImage}
                              alt={card.name}
                              className="w-20 h-20 object-cover flex-shrink-0 group-hover:scale-110 transition-transform duration-700"
                            />
                            <div className="p-3 flex-1 min-w-0 flex flex-col justify-center">
                              <p className="text-white text-[13px] font-bold truncate group-hover:text-[#ff5100] transition-colors">
                                {card.name}
                              </p>
                              <p className="text-white/30 text-[10px] font-bold uppercase tracking-widest truncate mt-0.5">
                                {card.state} · {card.type}
                              </p>
                              {rec?.reason && (
                                <p className="text-[#ff5100]/80 text-[11px] mt-1.5 line-clamp-2 leading-snug italic font-medium">
                                  &quot;{rec.reason}&quot;
                                </p>
                              )}
                            </div>
                            <ChevronRight className="w-4 h-4 text-white/10 group-hover:text-[#ff5100] self-center mr-4 flex-shrink-0 transition-all group-hover:translate-x-1" />
                          </Link>
                        );
                      })}
                    </div>
            ...
            {loading && (
              <div className="flex justify-start animate-in fade-in duration-300">
                <div className="bg-white/[0.04] border border-white/[0.08] px-5 py-3.5 rounded-2xl rounded-tl-none flex items-center gap-3">
                  <Loader2 className="w-4 h-4 text-[#ff5100] animate-spin" />
                  <span className="text-white/40 text-xs font-medium tracking-wide">Mapping optimal routes…</span>
                </div>
              </div>
            )}
            <div ref={bottomRef} />
          ...
          <div className="p-5 border-t border-white/[0.05] bg-white/[0.02]">
            <div className="relative flex items-center gap-2">
              ...
              <button
                ...
                className="p-4 rounded-xl transition-all shadow-xl shadow-[#ff5100]/10 disabled:opacity-20 disabled:grayscale hover:bg-[#ff7d47] active:scale-95 group"
