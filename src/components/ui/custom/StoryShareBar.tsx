...
const InstagramIcon = () => (
  <svg viewBox="0 0 24 24" className="w-4 h-4" fill="url(#igGradientBar)">
    <defs>
      <linearGradient id="igGradientBar" x1="0" y1="0" x2="24" y2="24">
...
export default function StoryShareBar({ title, slug }: Props) {
  const [copied, setCopied] = useState(false);
  const [igCopied, setIgCopied] = useState(false);
  const [url, setUrl] = useState("");

  useEffect(() => {
    setUrl(`${window.location.origin}/stories/${slug}`);
  }, [slug]);
    ...
    <div className="mt-8 pt-8" style={{ borderTop: "1px solid var(--border-subtle)" }}>
      <div className="flex flex-wrap items-center gap-2.5">
        ...
        <a
          ...
          style={{ background: "rgba(24,119,242,0.1)", border: "1px solid rgba(24,119,242,0.25)", color: "#1877F2" }}
        >
          <FacebookIcon />
          Facebook
        </a>
        ...
        <button
          ...
          style={{
            ...
            border: "1px solid rgba(214,36,159,0.25)",
            color: "#d6249f",
          }}
        >
          <InstagramIcon />
