...
export default function SavedAdventuresSection({ currentSlug }: { currentSlug: string }) {
  ...
  if (savedList.length === 0 && doneList.length === 0) {
    return (
      <section className="py-12 lg:py-16 px-5 lg:px-8" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3 mb-5">
            <Heart className="w-4 h-4 text-rose-400" />
            <p className="text-[10px] font-bold tracking-[0.22em] uppercase text-white/30">Your Adventures</p>
          </div>
          <p className="text-white/30 text-sm">
            No saved or completed adventures yet. Hit the <Heart className="w-3.5 h-3.5 inline text-rose-400" /> on any adventure to save it here.
          </p>
        </div>
      </section>
    );
