    ...
    >
      ...
      <div className="p-2.5">
        ...
        <div className="flex flex-wrap items-center gap-1 mt-1.5">
          ...
          {adventure.state && (
            <span className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-md bg-black/20 text-white/90 text-[9px] font-medium">
...
export default function ExploreMatchmakerSection() {
  ...
  if (!profile) {
    return (
      <div className="mt-12 lg:mt-16 border-t" style={{ background: "var(--bg-surface)", borderColor: "var(--border-subtle)" }}>
        <div className="max-w-7xl mx-auto px-5 lg:px-8 py-12 lg:py-16">
          <div className="relative rounded-2xl overflow-hidden px-8 py-10 flex flex-col md:flex-row md:items-center gap-8"
            ...
            {/* Decorative bg text */}
            <span className="pointer-events-none select-none absolute right-6 top-4 text-[80px] font-black leading-none hidden lg:block" style={{ color: "var(--text-muted)", opacity: 0.08 }}>ACE<sup>™</sup></span>
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex-1 flex flex-col gap-4 min-w-0">
            ...
            <div className="rounded-2xl overflow-hidden border relative"
              style={{ background: `linear-gradient(150deg, ${tier.color}12 0%, transparent 55%)`, borderColor: `${tier.color}22` }}>
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-[3px] mt-1.5">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <span key={i} className="text-[11px] leading-none" style={{ color: i < tier.stars ? tier.color : "var(--text-muted)" }}>★</span>
                      ))}
                      <span className="text-[9px] ml-1.5" style={{ color: "var(--text-muted)" }}>Rank {tier.stars} of 5</span>
                    </div>
                  </div>
                </div>
                ...
                {nextRank ? (
                  <div className="flex items-end justify-between mb-3">
                    <div>
                      ...
                      <p className="text-[10px] mt-1 leading-none" style={{ color: "var(--text-tertiary)" }}>to reach <span className="font-bold" style={{ color: nextRank.color }}>{nextRank.label}</span></p>
                    </div>
                    <div className="text-right">
                      <p className="text-xl font-black tabular-nums leading-none" style={{ color: "var(--text-secondary)" }}>{nextRank.minScore - totalScore}</p>
                      <p className="text-[10px] mt-1 leading-none" style={{ color: "var(--text-tertiary)" }}>pts needed</p>
                    </div>
                  </div>
                ) : (
          ...
          <div className="shrink-0 lg:self-stretch flex flex-col items-center rounded-2xl border p-5"
            ...
            <p className="text-[9px] uppercase tracking-[0.22em] font-bold self-start mb-3" style={{ color: "var(--text-muted)" }}>ACE<sup>™</sup> Profile</p>
