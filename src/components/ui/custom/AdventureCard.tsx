    ...
    >
      ...
      <div className="relative w-full overflow-hidden block group" style={{ height: isLarge ? "260px" : "200px" }}>
        ...
        <div className="px-4 pt-3 pb-4 flex flex-col gap-3">
          {/* Stats row */}
          <div className="flex flex-col gap-1.5 text-[11px] text-white/45 font-medium">
              <div className="flex items-center flex-wrap gap-x-2 gap-y-1">
                <span className="text-white/60">{adventure.duration}</span>
                {adventure.distance && (
                  <>
                    <span className="w-1 h-1 rounded-full bg-white/20" />
                    <span className="text-white/60">{adventure.distance}</span>
                  </>
                )}
                {adventure.altitude && (adventure.type === "Trekking" || adventure.type === "Biking" || adventure.type === "Mountaineering" || adventure.type === "Cycling") && (
                  <>
                    <span className="w-1 h-1 rounded-full bg-white/20" />
                    <span className="text-white/60">{adventure.altitude}</span>
                  </>
                )}
                <span className="w-1 h-1 rounded-full bg-white/20" />
                <span className="text-amber-400/70">{adventure.bestSeason}</span>
              </div>
              {(adventure.type === "Trekking" || adventure.type === "Biking") && (
                <div className="flex items-center gap-1.5 text-emerald-400/60">
                   <span className="truncate">{adventure.terrain}</span>
                </div>
              )}
          </div>
