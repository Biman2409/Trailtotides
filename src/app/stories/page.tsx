            ...
            <div className="relative h-[420px] lg:h-[540px] rounded-3xl overflow-hidden">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8 lg:p-12">
                {/* Badge pills (Featured, TTT Original) */}
                  <div className="flex items-center gap-2 mb-2">
                    {featured.tags.includes("Featured") && (
                      <span className="flex items-center gap-1.5 bg-black text-[#ff5722] text-xs font-semibold px-3 py-1.5 rounded-full shadow-lg border border-[#ff5722]/20">
                        <Crown className="w-3 h-3" /> Featured
                      </span>
                    )}
                      {featured.tags.includes("TTT Original") && (
                        <span className="flex items-center gap-1.5 bg-[#ff5722] text-black text-xs font-semibold px-3 py-1.5 rounded-full shadow-lg border border-[#ff5722]/20">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  <div className="w-8 h-8 rounded-full bg-[#ff5722]/40 flex items-center justify-center text-xs font-bold text-white flex-shrink-0 shadow-md shadow-[#ff5722]/20">
                    {featured.author[0]}
                  </div>
                  <div className="mr-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {rest.map((story) => (
              <Link key={story.id} href={`/stories/${story.slug}`} className="group block">
                ...
                <div>
                  <h3 className="text-[#1a1f2e] text-xl font-semibold leading-snug mb-2 group-hover:text-[#ff5722] transition-colors">
                    {story.title}
                  </h3>
