          ...
          <div className="lg:col-span-2">
            ...
            <AccordionSection label="Book This Adventure" title="" defaultOpen={true}>
              <div id="operators-section" className="space-y-3">
                ...
                {/* Compare Prices — inline under other operators */}
                    ...
                    <div style={{ background: "var(--bg-surface)" }}>
                      {allOperators
                        ...
                        .map((op, i, arr) => {
                          ...
                          return (
                            <div key={op.name} className="flex items-center gap-3 px-4 py-2.5" style={{ borderBottom: i < arr.length - 1 ? "1px solid var(--border-subtle)" : "none" }}>
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-1.5 mb-1">
                                  <span className="text-white/65 text-[11px] font-medium truncate">{op.name}</span>
                                  {isCheapest && <span className="text-[8px] font-bold px-1.5 py-0.5 rounded flex-none" style={{ background: "rgba(16,185,129,0.12)", color: "#34d399", border: "1px solid rgba(16,185,129,0.2)" }}>BEST</span>}
                                  {op.verified && <span className="text-[8px] font-bold px-1.5 py-0.5 rounded flex-none" style={{ background: "rgba(99,102,241,0.12)", color: "#a5b4fc", border: "1px solid rgba(99,102,241,0.2)" }}>✓</span>}
                                </div>
                                <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
                                  <div className="h-full rounded-full" style={{ width: `${pct}%`, background: isCheapest ? "#10b981" : op.verified ? "#6366f1" : "rgba(255,255,255,0.2)" }} />
                                </div>
                              </div>
