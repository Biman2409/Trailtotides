...
// public/india-boundary.geojson — used only as a faint security-watermark
// behind the bio-data page, the way real passports print a map/emblem ghost.
const INDIA_PATH =
    ...
    (
      <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", background: "radial-gradient(ellipse 90% 70% at 50% 15%, #7a1a26 0%, #4a0d16 55%, #2c0a10 100%)", padding: 36, position: "relative", fontFamily: "sans-serif" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "row", position: "relative" }}>
          ...
          {/* ── Left page — bio-data ─────────────────────────────── */}
          <div style={{ flex: 1, display: "flex", flexDirection: "column", background: PAPER, borderTopLeftRadius: 16, borderBottomLeftRadius: 16, padding: "42px 46px", position: "relative" }}>

            <svg width={420} height={480} viewBox="0 0 420 480" style={{ position: "absolute", left: "50%", bottom: -40, transform: "translateX(-50%)", opacity: 0.05 }}>
              <path d={INDIA_PATH} fill={INK} />
            </svg>
            ...
            <div style={{ display: "flex", flexDirection: "row", marginTop: 22, zIndex: 1 }}>
              {[
                { label: "TYPE", value: "P" },
            ...
            <div style={{ display: "flex", flexDirection: "column", marginTop: 26, zIndex: 1 }}>
              <span style={{ fontSize: 34, fontWeight: 800, letterSpacing: 5, color: INK }}>{"PASSPORT"}</span>
              <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: 3, color: INK, opacity: 0.45, marginTop: 4 }}>{"ADVENTURE & EXPEDITION"}</span>
            </div>

            <div style={{ display: "flex", flexDirection: "row", gap: 26, marginTop: 30, zIndex: 1 }}>
              <div style={{ width: 168, height: 208, display: "flex", alignItems: "center", justifyContent: "center", background: "#e9dfc3", border: `2px solid ${INK}`, borderRadius: 4, overflow: "hidden" }}>
                {avatarUrl ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={avatarUrl} width={168} height={208} style={{ objectFit: "cover" }} alt="" />
                ) : (
                  <div style={{ display: "flex", opacity: 0.25 }}>
                    <TypeIcon type="Mountaineering" size={64} color={INK} />
                  </div>
                )}
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", flex: 1, justifyContent: "center", gap: 16 }}>
                {[
                  ...
                  <div key={f.label} style={{ display: "flex", flexDirection: "column", borderBottom: "1px dashed rgba(36,26,18,0.22)", paddingBottom: 10 }}>
                    <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1.5, color: INK, opacity: 0.45 }}>{f.label}</span>
                    <span style={{ fontSize: 18, fontWeight: 700, color: INK, marginTop: 4 }}>{f.value}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* ── Right page — stamps ──────────────────────────────── */}
          <div style={{ flex: 1.05, display: "flex", flexDirection: "column", background: PAPER, borderTopRightRadius: 16, borderBottomRightRadius: 16, padding: "42px 44px", position: "relative", overflow: "hidden" }}>
            ...
            {stamps.length === 0 ? (
              ...
              <div style={{ display: "flex", flexWrap: "wrap", flexDirection: "row", justifyContent: "center", alignContent: "center", gap: 22, marginTop: 22, flex: 1 }}>
                {shown.map(({ adv, date, difficulty }, i) => {
                  const color = DIFFICULTY_COLOR[difficulty] ?? INK;
                  const d = parseISODate(date);
                  return (
                    <div key={adv.slug} style={{ display: "flex", width: size, height: size, borderRadius: 999, border: `3px solid ${color}`, alignItems: "center", justifyContent: "center", position: "relative", transform: `rotate(${STAMP_ROTATIONS[i % STAMP_ROTATIONS.length]}deg)` }}>
                      <div style={{ position: "absolute", inset: 7, display: "flex", borderRadius: 999, border: `1.5px dashed ${color}`, opacity: 0.65 }} />
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: 8 }}>
                        <div style={{ display: "flex" }}>
                          <TypeIcon type={adv.type} size={size >= 150 ? 24 : 18} color={color} />
                        </div>
                        <span style={{ fontSize: size >= 150 ? 13 : 11, fontWeight: 800, color, marginTop: 6, textAlign: "center", lineHeight: 1.15 }}>
                          {truncate(adv.name, size >= 150 ? 20 : 16)}
                        </span>
                        <span style={{ fontSize: size >= 150 ? 10 : 9, fontWeight: 700, color, opacity: 0.7, marginTop: 4, letterSpacing: 0.5 }}>
                          {fmtDate(d)}
                        </span>
                      </div>
                    </div>
                ...
                {overflow > 0 && (
                  <div style={{ display: "flex", width: size, height: size, borderRadius: 999, border: `3px solid ${INK}`, alignItems: "center", justifyContent: "center", opacity: 0.55, transform: `rotate(${STAMP_ROTATIONS[shown.length % STAMP_ROTATIONS.length]}deg)` }}>
                    <span style={{ fontSize: size >= 150 ? 22 : 18, fontWeight: 800, color: INK }}>{`+${overflow}`}</span>
                  </div>
                )}
              </div>
            )}
