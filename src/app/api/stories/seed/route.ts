...
export async function POST() {
  try {
    ...
    for (const s of staticStories) {
      ...
      const body = getStoryBody(s.slug);
      ...
      if (error) {
        console.error(`seed error for ${s.slug}:`, error);
      } else {
        inserted++;
      }
    }

    const body = `/api/stories/seed - DONE: ${inserted} inserted, ${skipped} skipped`;
    console.log(body);
...
// Copy the story bodies from the slug page logic
...
function getStoryBody(slug: string): string | null {
  return STORY_BODIES[slug] ?? null;
}