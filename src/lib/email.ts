    ...
    const { data, error } = await resend.emails.send({
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;">
          <h1 style="color: #f4845f;">Welcome to the Wild, ${name}!</h1>
          ...
          <div style="margin: 30px 0;">
            <a href="${link}" style="background-color: #f4845f; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: bold; display: inline-block;">
          ...
          <p style="font-size: 14px; color: #f4845f; word-break: break-all;">
            ${link}
          </p>
...
export async function sendPasswordResetEmail(email: string, link: string) {
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;">
          <h1 style="color: #f4845f;">Password Reset Request</h1>
          <p style="font-size: 16px; line-height: 1.5;">
