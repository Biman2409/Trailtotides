...
-- Run this in your Supabase SQL Editor (https://supabase.com/dashboard/project/eylgddhfxzxwovcodihx/sql/new)
-- ============================================================

-- 0. Storage Buckets
-- Create these manually in Supabase Dashboard > Storage:
--   story-submissions  (public)
--   story-data         (private)
--   story-reactions    (public)
--   story-comments     (public)
--   adventure-images   (public)
--   adventure-photos   (public, allowed MIME types: image/jpeg, image/png, image/webp, image/heic, max 8MB)
--   operator-profiles  (private)
--   operator-submissions (private)
--   wishlists          (private)
--   user-data          (private)
-- Or run:
-- select storage.create_bucket('story-submissions', 'public');
-- select storage.create_bucket('story-data', 'private');
-- etc.
...
-- Stored procedure for incrementing story views
...
INSERT INTO public.story_views (slug, views) VALUES
  ('the-night-photi-la-tested-us', 2553),
  ('riding-through-a-revolution', 2532)
...
DO $$ BEGIN
  CREATE POLICY "Service role read all" ON public.contact_messages FOR SELECT TO service_role USING (true);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  CREATE POLICY "Operators view own profile" ON public.operator_profiles FOR SELECT USING (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Operators insert profile" ON public.operator_profiles FOR INSERT WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Operators view own submissions" ON public.operator_submissions FOR SELECT USING (
    operator_id IN (SELECT id FROM public.operator_profiles WHERE user_id = auth.uid())
  );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Operators insert submissions" ON public.operator_submissions FOR INSERT WITH CHECK (
    operator_id IN (SELECT id FROM public.operator_profiles WHERE user_id = auth.uid())
  );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Service role full access profiles" ON public.operator_profiles FOR ALL TO service_role USING (true) WITH CHECK (true);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Service role full access submissions" ON public.operator_submissions FOR ALL TO service_role USING (true) WITH CHECK (true);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  CREATE POLICY "Users view own saved" ON public.saved_adventures FOR SELECT USING (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Users insert own saved" ON public.saved_adventures FOR INSERT WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Users delete own saved" ON public.saved_adventures FOR DELETE USING (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  CREATE POLICY "Users insert own rating" ON public.operator_ratings FOR INSERT WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Users view own rating" ON public.operator_ratings FOR SELECT USING (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Service role full access" ON public.operator_ratings FOR ALL TO service_role USING (true) WITH CHECK (true);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;